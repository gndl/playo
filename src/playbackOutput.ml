...
let openOutput =
	try Some (Ao.open_live)
	with x -> printf "faile to open device"; None
...
class c ?(name = "Playback Output(") () =
	...
	method write data lg =
		let wrt device =
			let buf = S.create (lg * mSampleSize) in
		...
		match mDevice with
			| Some dvc -> wrt dvc
			| None -> (self#openOutput (A.length channels); match mDevice with
				| Some dvc -> wrt dvc
				| None -> ())
...
(*
	method write time buffer lg channels =
		let wrt device =
			...
			let buf = S.create (lg * sampleSize) in
