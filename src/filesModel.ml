	...
	method filter regexp =
		...
		let rec fltr visible row =
			...
			| Dir dir -> A.fold_left ~f:fltr ~init:false dir.children
					|| (try ignore(Str.search_forward regexp (String.lowercase_ascii row.name) 0);
			  				true
  						with Not_found -> false)
			| File f -> (try
  				ignore(Str.search_forward regexp (String.lowercase_ascii f.title) 0);
					true
  			with Not_found -> try
  				ignore(Str.search_forward regexp (String.lowercase_ascii f.artist) 0);
  				true
  			with Not_found -> try
  				ignore(Str.search_forward regexp (String.lowercase_ascii f.album) 0);
  				true
  			with Not_found -> try
  				ignore(Str.search_forward regexp (String.lowercase_ascii row.name) 0);
  				true
  			with Not_found -> try
  				ignore(Str.search_forward regexp (String.lowercase_ascii f.genre) 0);
  				true
  			with Not_found -> false)
			| Null -> false);
